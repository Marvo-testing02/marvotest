...
import dottedLine from '../../assets/Ways/FourWay.svg';
import sideIllustration from '../../assets/illustrations/salesConsulting.svg';
import '../../App.css';

const EnterprisePage4 = () => {
    ...
    return (
        ...
        >
            ...
            <Box sx={{ position: 'absolute', top: 43, left: 172, width: 595, height: 124, zIndex: 5 }}>
                <Box component="img" src={dottedLine} width="100%" />
            </Box>
            <motion.div
                initial={{ top: 45, left: 528.18 }}
                animate={{ top: 85.86, left: 714.61 }}
            ...
            <Box sx={{ position: 'absolute', top: 125, left: 350, width: 35, height: 35, borderRadius: '50%', backgroundColor: '#57AFFF', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 16, zIndex: 7 }}>
                2
            </Box>
            <Box sx={{ position: 'absolute', top: 80, left: 550, width: 35, height: 35, borderRadius: '50%', backgroundColor: '#57AFFF', color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 16, zIndex: 7 }}>
                3
            </Box>
            {[{ num: 5, x: 926, y: 30 }, { num: 6, x: 1158, y: 147 }].map((step, i) => (
                <Box key={i} sx={{ position: 'absolute', top: step.y, left: step.x, width: 35, height: 35, borderRadius: '50%', backgroundColor: '#BBDFFF', color: '#1A6DF6', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: 16, zIndex: 2 }}>
                    {step.num}
                </Box>
            ))}
            <motion.div
                ...
                >
                    ...
                    >
                        <Typography variant="h4" fontWeight="bold" color="#1254DC" mb={3}>
                            Which departments will you use MARVO?
                        </Typography>

                        <RadioGroup value={goal} onChange={(e) => setGoal(e.target.value)}>
                            {['Sales', 'Marketing', 'Customer Support', 'HR', 'All'].map((option, i) => (
                                <FormControlLabel
                                    key={i}
                                    value={option}
                                    ...
                                    label={
                                        <Typography
                                            className={skipped && goal === '' ? 'glow-animation' : ''}
                                            sx={{
                                                fontWeight: 'bold',
                                                color: '#2B2B2B',
                                                fontSize: 18,
                                                ...
                                                py: 1,
                                                borderRadius: 0,
                                                border: skipped && goal === '' ? '2px solid #31D6FF' : 'none',
                                                backgroundColor: skipped && goal === '' ? 'rgba(49, 214, 255, 0.2)' : 'transparent',
                                            }}
                                        >
                                            {option}
                                        </Typography>
...
export default EnterprisePage4;
